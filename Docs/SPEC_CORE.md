...
**2026-06-07 — Motion Matching action pack (+23: animation +14, chooser +3, blueprint +6):** End-to-end UE 5.7 Motion Matching authoring surface. `animation` +14 (PoseSearch normalization sets / asset-type-agnostic database entries / schema mirroring + channels / notifies / validation, plus the Pose-History + Motion-Matching anim-graph node configure/build actions: `create_normalization_set`, `add_database_to_normalization_set`, `set_database_normalization_set`, `add_database_entry`, `set_database_entry_tags`, `create_mirror_data_table`, `set_schema_mirror_data_table`, `configure_schema_channel`, `add_pose_search_notify` (8 notify-state kinds), `derive_schema_channels_from_skeleton`, `validate_pose_search_database`, `configure_pose_history_node`, `configure_motion_matching_node`, `build_motion_matching_node` — see [`specs/SPEC_MonolithAnimation.md` § PoseSearch / Motion Matching action pack](specs/SPEC_MonolithAnimation.md)). `chooser` +3 (`create_chooser_table` w/ `output_class` resolution, `add_chooser_column` Bool/Enum/GameplayTag/FloatRange/OutputObject, `add_chooser_row` — chooser table authoring; row 6 → 9 — see [`specs/SPEC_MonolithAnimation.md` § Chooser Namespace](specs/SPEC_MonolithAnimation.md)). `blueprint` +6 (`set_anim_class`, `apply_movement_preset`, `add_engine_component_typed`, `scaffold_locomotion_input`, `validate_animbp_variable_contract`, `scaffold_motion_matching_character` (composite) — Motion-Matching character scaffolding; see [`specs/SPEC_MonolithBlueprint.md`](specs/SPEC_MonolithBlueprint.md)). No-count behavior: `add_anim_graph_node` gained `pose_history` + `inertialization` aliases; `get_database_stats` + `get_pose_search_schema` gained read-back fields. **Fixes:** pre-existing crash in `get_database_stats` (asserted on unbuilt PoseSearch databases); pre-existing crash in `add_anim_graph_node` when spawning bound-graph nodes (BlendStack / MotionMatching — now uses `FGraphNodeCreator`). **New Build.cs deps:** `MonolithAnimation += BlendStackEditor`; `MonolithBlueprint += EnhancedInput`. All 23 are PUBLIC, in-tree — **zero sibling-plugin actions** (per `monolith-release.md` Action Count Discipline). In-tree total moves from ~1427 toward **~1450** (the 14 PoseSearch + 6 blueprint additions are always-on; the 3 chooser additions are `WITH_CHOOSER`-gated). Namespace count unchanged at 26 in-tree (30 total). Per the approximate-count note at the top of §12, exact integers are no longer tracked — query `monolith_discover()` for the live figure.
