...
| **Total** | | **1358** in-tree active actions across 19 in-tree namespaces (15 module namespaces + 4 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`). Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. 45 town-gen experimental actions disabled by default → 1403 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1303 (1348 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1223 (1268 w/ town gen — strips the 135 GAS actions); without both: 1168 (1213 w/ town gen). Without `WITH_METASOUND`: 1346 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1354** in default-active configuration, **1399** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` (which includes siblings if installed) returns **1581** = 1358 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8) and 19 → 19 with v0.14.11 [Unreleased] (`bulk_fill` + `describe` were already counted as Phase 0 dispatchers; this release lights up their adapters but does not add new namespaces). The 2026-05-22 `monolith_guide` addition leaves the namespace count at 19 (new action in the existing `monolith` meta-namespace). Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |
