...
- **Action counts (live):** v0.17.0 in-tree total lands at **1384 actions across 24 in-tree namespaces** (Phases 1+2+3a = +16 actions / +3 namespaces; Phase 4a = +10 actions / +2 namespaces over Phase 3a's 1374 / 22). Live `monolith_discover()` returns **1607 total across 28 namespaces** with all four host-project sibling plugins loaded (1384 in-tree + 11 `claudedesign` + 158 `inventory` + 28 `steam` + 26 `substance` = 1607). Sibling action counts remain advisory and never affect the public Monolith release headline.
