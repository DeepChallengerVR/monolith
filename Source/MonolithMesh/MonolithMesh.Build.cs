...
public class MonolithMesh : ModuleRules
{
	public MonolithMesh(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (bHasGeometryScripting)
		{
			...
			// Delay-bind the GeometryScripting module DLLs on Windows so MonolithMesh.dll
			// loads even if these aren't resolvable at module-load time. macOS links
			// dylibs directly; adding Windows .dll names there makes ld search for
			// impossible Engine/Source/*.dll paths.
